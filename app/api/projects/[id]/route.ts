    ...
    try {
        ...
        if (!githubResponse.ok) {
            if (githubResponse.status === 404) {
                return NextResponse.json({ message: 'Репозиторій не знайдено або став приватним' }, { status: 404 });
            }
            ...
            return NextResponse.json({ message: 'Помилка при оновленні даних з GitHub' }, { status: 500 });
        }
        ...
        return NextResponse.json({ message: 'Помилка при оновленні проєкту' }, { status: 500 });
    }
}
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        return NextResponse.json({ message: 'Помилка при видаленні проєкту' }, { status: 500 });
    }
}
